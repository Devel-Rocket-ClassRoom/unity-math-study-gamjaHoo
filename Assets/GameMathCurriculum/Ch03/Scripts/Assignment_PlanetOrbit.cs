...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    {
        // TODO
        planetAngle += planetOrbitSpeed * Time.deltaTime;
        satelliteAngle += satelliteOrbitSpeed * Time.deltaTime;
        if(planetAngle > 360f)
        {
            planetAngle -= 360f;
        }
        if(satelliteAngle > 360f)
        {
            satelliteAngle -= 360f;
        }
        ...
        planetWorldPos = planetMatrix.MultiplyPoint3x4(planetOffset);
        transform.position = planetWorldPos;

        Matrix4x4 satelliteMatrix = Matrix4x4.TRS(planetWorldPos, Quaternion.Euler(0f, satelliteAngle, 0f), Vector3.one);

        Vector3 satelliteOffset = new Vector3(satelliteOrbitRadius, 0f, 0f);

        satelliteWorldPos = satelliteMatrix.MultiplyPoint3x4(satelliteOffset);
