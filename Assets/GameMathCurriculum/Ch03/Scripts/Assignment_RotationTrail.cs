...
public class Assignment_RotationTrail : MonoBehaviour
{
    ...
    private List<Vector3> trailPositions = new List<Vector3>(); //얘로 궤적의 위치들을 저장할거임
    ...
    private void Update()
    {
        ...
        Matrix4x4 rotationMatrix = Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0f, 0f, rotationAngle), Vector3.one);
        Vector3 offSet = new Vector3(armLength, 0f, 0f);

        transform.position = rotationMatrix.MultiplyPoint3x4(offSet);
