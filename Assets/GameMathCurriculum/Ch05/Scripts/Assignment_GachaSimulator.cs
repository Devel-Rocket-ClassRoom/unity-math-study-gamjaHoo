...
public class Assignment_GachaSimulator : MonoBehaviour
{
    ...
    {
        pulledSSR = false;
        while (currentPityCount < hardPity && !pulledSSR)
        {
            ExecutePull();
        }
    }
    ...
    private void ExecutePull()
    {
        ...
        {
            float t = (float)(currentPityCount - softPityStart) / (hardPity - softPityStart);
            currentEffectiveRate = Mathf.Lerp(baseRate, 1f, t);
        }
        else
        {
        ...
        {
            totalSSRs++;
            ssrPityList.Insert(0, currentPityCount + 1); // 최신 순으로 앞에 삽입
            currentPityCount = 0;                         // 천장 리셋
            pulledSSR = true;
        }
        else
        {
