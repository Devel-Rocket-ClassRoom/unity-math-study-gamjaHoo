...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < units.Length; i++)
        {
            ...
            Quaternion unitRot = Quaternion.AngleAxis(angle, Vector3.up);
            // 리더 회전 합성
            Quaternion composed = leader.rotation * unitRot;

            Vector3 offset = composed * Vector3.forward * formationRadius;

            //Quaternion slotRotation = Quaternion.AngleAxis(angle, leader.up);
            //Vector3 offset = slotRotation * Vector3.forward * formationRadius;

            units[i].position = leader.position + offset;
            units[i].rotation = composed;   // 리더와 같은 방향을 바라봄
        }
        UpdateUI();
    }
