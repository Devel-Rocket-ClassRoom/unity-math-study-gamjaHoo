...
public class Assignment_SplineConveyor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for(int i = 0; i < boxes.Length; i++)
        {
            ...
            float boxT = (globalT + offset) % 1f;

            boxes[i].position = EvaluateSpline(waypoints, boxT);

            float nextT = (boxT + 0.00001f) % 1f;
            Vector3 tangent = (EvaluateSpline(waypoints, nextT) - boxes[i].position).normalized;
            if(tangent != Vector3.zero)
            {
                boxes[i].rotation = Quaternion.LookRotation(tangent);
            }
